...
pair_t pair_new(int x, int y){
    pair_t p = malloc(sizeof(struct s_pair_t));  //Pregunta: Como devuelve un puntero, si no hago malloc,
                                                //devuelvo una direccion de memoria que no tiene nada (se borra el stack)
    p->fst = x;
    p->snd = y;
    return p;
}
