...
int main(int argc, char *argv[]) {
    ...
    /* show data analysis */
    fprintf(stdout, "Minima temperatura registrada: %d \n", temp_min(table));
    
    int array_maxtemps [YEARS];
    month_t array_rainmonths [YEARS];
    
    max_temp_array(table, array_maxtemps);
    rainfall_month_array(table, array_rainmonths);
    
    fprintf(stdout, "Año  | maxTemp | mes mas lluvioso\n");
    
    for (unsigned int year = 0u; year < YEARS; ++year) {
        
        fprintf(stdout, "%u     %u     %d \n", year+1980, array_maxtemps[year], array_rainmonths[year]+1);
    }
